...
export default function Calendar() {
  ...
  // Deep link support: /calendar?openAdd=1&date=YYYY-MM-DD
  useEffect(() => {
  ...
  const addActivity = (activityData) => {
    const effectiveDate =
      activityData?.date instanceof Date && !isNaN(activityData.date)
        ? activityData.date
        : selectedDate
  ...
  const getActivityDetails = (activity) => {
    if (activity.type === 'outfit') {
      ...
      return {
        ...
        items: OutfitService.getClothesInOutfit(activity.outfitId).map(
          (c) => c.name
        ),
      }
    } else {
      ...
      return {
        ...
        items: activity.clothIds
          .map((id) => ClothService.getById(id)?.name)
          .filter(Boolean),
      }
    }
  }
              ...
              {activities[formatDate(selectedDate)]?.length > 0 ? (
                <div className="space-y-3">
                  {activities[formatDate(selectedDate)].map((activity) => {
                    ...
                    return (
                      <div
                        key={activity.id}
                        className="flex items-start gap-3 p-3 bg-gray-50/50 dark:bg-gray-700/50 rounded-lg"
                      >
                        <div
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-600 dark:text-gray-400 text-sm mb-2">
                    No outfits logged for this date
                  </p>
                  <p className="text-gray-500 dark:text-gray-500 text-xs">
                    Long press on a date to add an activity
                  </p>
                </div>
              )}
            </div>
