...
export default function CategoriesView({ onBack }) {
  ...
  const [modalData, setModalData] = useState({ parentId: null, id: null, name: '', maxWearCount: 2 });
  ...
  const openDeleteModal = (category) => {
    const count = AdvancedAnalyticsService.getNumberOfClothesByCategory(category.id, getSubcategories(category.id));
  ...
  const handleReassignAndDelete = () => {
    ...
    const categoryIds = [reassignTarget.id, ...subcategories.map(cat => cat.id)];
    AdvancedAnalyticsService.moveClothesToCategory(categoryIds, reassignCategoryId);
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 pb-24">
              ...
              >
                ...
                <div className="mb-4">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      CategoryService.update(category.id, { maxWearCount: parseInt(e.target.value) }) ||
                      loadCategories()
                    }
                    min="1"
                    max="10"
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    {getSubcategories(category.id).map((sub) => (
                      ...
                      >
                        <div>
                          <span className="text-sm text-blue-700 dark:text-blue-300">{sub.name}</span>
                          <span className="ml-2 text-xs text-gray-500 dark:text-gray-400">
                            (max {sub.maxWearCount ?? getEffectiveMaxWearCount(category.id)})
                          </span>
                        </div>
                        <div className="flex gap-2">
                          <Button
      ...
      <Modal
        ...
        title={modalData.id ? 'Edit Category' : modalData.parentId ? 'Add Subcategory' : 'Add Category'}
        size="sm"
        footer={
        ...
        <div className="space-y-4">
          ...
          <div>
            <Input
              ...
              onChange={(e) => setModalData({ ...modalData, maxWearCount: e.target.value })}
            />
            {errors.maxWearCount && <p className="text-xs text-red-500 mt-1">{errors.maxWearCount}</p>}
          </div>
        </div>
      </Modal>
      ...
      <Modal
        ...
        footer={
          <div className="flex justify-end gap-2">
            ...
            <Button disabled={!reassignCategoryId} onClick={handleReassignAndDelete}>
              Reassign & Delete
            </Button>
          </div>
        }
      >
        <p className="text-gray-700 dark:text-gray-300 mb-4">
          <span className="font-semibold">{reassignTarget?.name}</span> has clothes assigned.  
          Please select a new category to move them before deleting.
        </p>
        <select
