            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-6">
                    ...
                    <div className="pt-2">
                        <button type="button" onClick={() => setShowAdvanced(!showAdvanced)} className="flex items-center justify-between w-full text-left font-semibold text-gray-800 dark:text-gray-200">
                            ...
                            <ChevronsUpDown size={20} className={`transition-transform ${showAdvanced ? 'rotate-180' : ''}`} />
                        </button>
                        {showAdvanced && (
                                ...
                                <FormField label="Color">
                                    <div className="flex flex-wrap gap-2">
                                        {predefinedColors.map(color => (
                                            <button key={color} type="button" title={color} onClick={() => { handleInputChange('color', color); setShowCustomColor(false); }} className={`w-9 h-9 rounded-full border-2 transition-transform hover:scale-110 focus:outline-none ${formData.color === color ? 'border-blue-500 ring-2 ring-blue-500 ring-offset-2 ring-offset-white dark:ring-offset-gray-900' : 'border-gray-300 dark:border-gray-700'}`} style={{ backgroundColor: color }} aria-label={color} />
                                        ))}
                                        <button type="button" title="Custom Color" onClick={() => { handleInputChange('color', 'custom'); setShowCustomColor(true); }} className={`w-9 h-9 flex items-center justify-center rounded-full border-2 transition-transform hover:scale-110 focus:outline-none ${showCustomColor ? 'border-blue-500 ring-2 ring-blue-500 ring-offset-2 ring-offset-white dark:ring-offset-gray-900' : 'border-gray-300 dark:border-gray-700'}`}>
                ...
                <div className="pt-2 flex gap-3 justify-end">
                    ...
                    <Button type="submit" disabled={isLoading} className="w-full sm:w-auto flex items-center justify-center gap-2">
                        {isLoading ? (<><Loader2 size={18} className="animate-spin" /> Adding...</>) : 'Add Item'}
                    </Button>
                </div>
            </form>
        </Modal>
